	...
	if(isPlayer leader(group _killer))then{
		...
		if(_HALV_KillFeed_AI)then{
			...
			if(_killerName isEqualTo "Error: No vehicle")then{
				_nearkillerobjs = nearestObjects [_player,["CIVILIAN","civilian","Civilian"],2500];
...
//				if(typeName _killer isEqualTo "OBJECT")exitWith{};
			...
			_message = format["%1 was wounded by %2 with %3 from %4m",_victimName,_killerName,_txt,_distance];
			_hint = _message;
	...
	//if player is killer
	_message = format["%1 was killed ...",name _player];
	_hint = format["<t align='Center'shadow='2'>%1</t>",_message];
	_dyntxt = format["<t size='0.70'align='left'shadow='1'>%1</t>",_message];
	_loc_message = format["[KillFeed]: PKILL: %1 was killed | '%2' - '%3'", _victimName,_killer,_weapon];
};
