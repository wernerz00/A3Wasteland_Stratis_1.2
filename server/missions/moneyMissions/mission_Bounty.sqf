...
_missionHintText = format ["<t align='center' color='%2' shadow='2' size='1.75'>Bounty Hunt</t><br/><t align='center' color='%2'>------------------------------</t><br/><t color='%3' size='1.0'>%1 on %4 has a bounty on his head. You have 30 minutes to kill him! Killer gets $85,000 and his side gets $5,000 per person. If he's protected he gets the $85,000 and his side gets $5,000 per person.</t>", name _foundPlayer, bountyMissionColor, subTextColor, _playerSideName];
//messageSystem = _hint;
//if (!isDedicated) then { call serverMessage };
//publicVariable "messageSystem";
...
[format ["addBounty:%1:%2:%3:%4:%5:%6:%7:%8", call A3W_extDB_ServerID, call A3W_extDB_MapID, 'SYSTEM', getPlayerUID _foundPlayer, '0', _mission_state, 5000, 85000]] call extDB_Database_async;
...
_failedExec =
{
	...
	if (_mission_state == BOUNTY_MISSION_END_SUICIDE) then {
		_failedHintMessage =  format ["<t align='center' color='%2' shadow='2' size='1.75'>Objective Failed</t><br/><t align='center' color='%2'>------------------------------</t><br/><t align='center' color='%3' size='1.25'>FAIL!!</t><br/><br/><t align='center' color='%3'>%1 took the coward's way out and committed suicide!</t>", _playerName, failMissionColor, subTextColor];
	...
	if (_mission_state == BOUNTY_MISSION_END_DISCONNECT) then {
		_failedHintMessage =  format ["<t align='center' color='%2' shadow='2' size='1.75'>Objective Failed</t><br/><t align='center' color='%2'>------------------------------</t><br/><t align='center' color='%3' size='1.25'>FAIL!!</t><br/><br/><t align='center' color='%3'>%1 took the coward's way out and disconnected! He will become the bounty when he reconnects!</t>", _playerName, failMissionColor, subTextColor];
		[format ["updateBounty:%1:%2:%3:%4", _destPlayerUID, '0','0', _mission_state]] call extDB_Database_async;
	};
